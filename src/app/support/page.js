      ...
      <div className="grid md:grid-cols-2 gap-8">
        {supportPrograms.map((program, index) => (
          <Link href={program.link} key={index}>
            <div className={`${program.color} rounded-lg shadow-lg p-6 transform transition duration-300 hover:scale-105 cursor-pointer`}>
              <div className="flex items-center mb-4">
                {program.icon}
                <h2 className="text-xl font-semibold ml-2">{program.title}</h2>
              </div>
              <p className="text-gray-600 mb-4">{program.description}</p>
              <div className="flex items-center text-indigo-600 font-semibold hover:text-indigo-800">
